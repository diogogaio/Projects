...
import { useLocalBaseContext, useThemeContext } from "../../contexts";
...
export const LogTrainingMd = React.memo(
  ({ setIsTodo, selectedCellId, setSelectedCellId }: TLogTrainingMd) => {
    ...
    const createDefaultChartsButtons = () => {
      const buttons = defaultGymCharts.map((c) => (
        <Button
          ...
          variant="outlined"
          onClick={(e) =>
            handleLogTraining(e.currentTarget.id, defaultGymCharts)
          }
        >
          {c.tittle}
        </Button>
          ...
          <Box component="form" noValidate sx={style}>
            ...
            {Utils.isDefaultChart() ? (
              <Typography component="aside" variant="captionXS">
                {" "}
                Após realizar o comentário, selecione uma ficha padrão para
                salvar o treino ou{" "}
