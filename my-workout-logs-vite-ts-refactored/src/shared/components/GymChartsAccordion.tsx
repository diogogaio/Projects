...
type TGymChartsAccordion = {
  ...
  callBack?: (id: string, gymCharts: IAppStateType["gymCharts"]) => void;
};
...
export const GymChartsAccordion = ({
  ...
  const Actions = ({ tittle, id }: TActionsProps) => {
    return deletable ? (
      ...
      <Link
        ...
        onClick={(e) => {
          e.stopPropagation();
          if (callBack) callBack(e.currentTarget.id, gymCharts);
        }}
      >
        Selecionar
      </Link>
  ...
  const createAccordion = useCallback(
    ...
    [gymCharts, expanded, showTodoList, calendarData]
  );
